<template>
  <div class="task_router_content" @touchstart.stop="testDown" @touchend.stop="touchtUp" @touchmove.stop="touchMove" @mouseout.stop="touchStart = false">
    ...
    <div class="list_wrap" ref="down_section">
      <div class="inner-head">
        ...
        <v-spacer></v-spacer>
        <v-text-field label="Search" clearable prepend-inner-icon="mdi-magnify" class="min_hide" outlined dense style=" flex:150px 0 0; height: 40px"></v-text-field>
        <v-select v-model="detail.association" :items="selects" clearable outlined dense label="Association" class="min_hide ml-5" style="flex:150px 0 0; height: 40px"></v-select>
        <!--  -->
          ...
          <v-list subheader style="background: none">
            <div class="v-list-item-pro" v-for="(item, key) in items" :key="key" :class="'v-list-item-' + item.className">
              <v-list-item :class="{ active: item.active }" @click.stop="() => {}" :ripple="false">
                <v-list-item-action>
                  <v-checkbox v-model="item.selected" color="primary" @change="itemSelectedChange(key)"></v-checkbox>
                </v-list-item-action>
    ...
    <!--右边部分开始-->
    <div class="right_section" ref="right_section" :class="{ hidetaskdetail: detailStatus }" @click.stop="() => {}">
      <v-card class="inner_wrap" v-if="!detailStatus">
        <v-toolbar style="width: 100%" absolute>
          <v-btn rounded :color="detail.selectedColor" outlined class="mr-5" @click.stop="handleCheckbox(detail.key)">
          ...
          <div class="content">
            <v-layout row wrap style="margin: 20px 20px 0 0">
              ...
              <v-flex xs5>
                <v-menu offset-y>
                  <template v-slot:activator="{ on }">
                    <v-text-field v-model="detail.startTime" v-on="on" label="Start time" required style="margin-right: 10px"></v-text-field>
                  </template>
              ...
              <v-flex xs5>
                <v-menu offset-y>
                  <template v-slot:activator="{ on }">
                    <v-text-field v-model="detail.endTime" v-on="on" label="End time" required style="margin-left: 10px"></v-text-field>
                  </template>
                  <v-date-picker v-model="detail.endTime"></v-date-picker>
                </v-menu>
              </v-flex>
            </v-layout>
            <v-tabs v-model="tab" style="border-bottom: 1px solid rgba(0,0,0,0.12); min-width: 320px; background: #fff">
    ...
    <v-dialog v-model="createTaskmDialogVisible" :persistent="false" max-width="600px">
      <v-card>
        ...
        <v-form ref="form" v-model="valid" lazy-validation class="task_form">
          <v-text-field v-model="detail.name" :counter="10" :rules="nameRules" label="Task name" required></v-text-field>
  ...
  data() {
    return {
      ...
      nameRules: [
        ...
        (v) => (v && v.length <= 10) || 'Task name must be less than 10 characters',
      ],
      loadText: '下拉刷新',
      valid: true,

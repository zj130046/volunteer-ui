<template>
  <div class="login_container" :class="{ login_mlmlh: mlmlh }">
    ...
    <div class="wrap">
      <v-card class="inner_card">
        ...
        <div class="left">
          ...
          <div class="row row_f">
            <v-btn block x-large rounded color="primary" @click="handleSignBtn"
              >let's go</v-btn
            >
          </div>
        </div>
        <v-dialog v-model="dialog" width="400" persistent>
          <v-card class="mx-auto" max-width="400" height="150" outlined>
            <v-card-title>
              <v-icon>iconfont vo-xinxi</v-icon>
            </v-card-title>
            <v-card-text
              >您正在登陆Super Admin账号，点击确定获取动态密码</v-card-text
            >
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn
              ...
              <v-btn color="primary" text @click="send"> 发送 </v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-card>
    ...
    <div class="bottomBox">
      ...
      <hr />
      <div>
        ©2017 - 2021 武汉科技大学青年志愿者服务总队. All Rights Reserved. ICP证:
